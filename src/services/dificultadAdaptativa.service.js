...
export const calculateAdaptiveDifficulty = ({ stats, recentSessions = [], minigame }) => {
  ...
  if (!evidenceReady) {
    return buildDecision({
      ...
      decision: 'mantener',
      reason:
        'Mantiene el nivel hasta reunir al menos dos sesiones recientes y ocho intentos acumulados.',
      metrics,
    });
  }
  ...
  if (inCooldown) {
    return buildDecision({
      ...
      decision: 'mantener',
      reason:
        'Mantiene el nivel para completar el periodo de estabilizacion despues del ultimo cambio.',
      metrics,
    });
  }
...
export const calculateInActivityDifficulty = ({ previousSession, minigame }) => {
  ...
  if (attempts <= 0) {
    return buildDecision({
      ...
      decision: 'mantener',
      reason:
        'Mantiene el nivel porque la mision anterior no registro intentos validos.',
      metrics: {
        ...commonMetrics,
  ...
  return buildDecision({
    ...
    decision: 'mantener',
    reason:
      'Mantiene el nivel porque completo la mision con un margen de practica adecuado.',
    metrics: commonMetrics,
  });
};
