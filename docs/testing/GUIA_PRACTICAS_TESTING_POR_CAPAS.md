...
## Como preparar tu rama y README

Cada equipo o integrante debe trabajar en su propia rama segun el area que le
toque. No se debe trabajar directo sobre `develop`.

Ramas sugeridas:

1. `TEST-PRUEBAS-FRONTEND-COMPLETA`
2. `TEST-PRUEBAS-MOVIL-COMPLETA`
3. `TEST-PRUEBAS-BACKEND-AJUSTES`, solo si en el futuro se modifica backend

En esa rama deben crear o actualizar un `README.md` tomando como ejemplo el
README de esta rama backend. No se debe copiar palabra por palabra si el area es
frontend o movil; se debe adaptar.

El README de cada area deberia explicar:

1. que area se probo
2. que herramienta se uso
3. que capas se definieron
4. que modulos o pantallas se cubrieron
5. que comandos ejecutan las pruebas
...
```text
Rama: TEST-PRUEBAS-FRONTEND-COMPLETA
Area: frontend
Herramienta: Vitest + Testing Library o Playwright
Capas: rutas, formularios, roles, API, componentes criticos
Comando principal: npm run test o el que defina el proyecto frontend
Evidencia: README + guia + reporte de estado
```
